...
void solve(i32 n) {
	i32 bound = sqrt(n);
	vector<i32> counts(n, 0);
	for (auto x : iota(1, bound)) {
		for (auto y : iota(x, bound)) {
			for (auto z : iota(y, bound)) {
				auto w = x * x + y * y + z * z + x * y + y * z +
					 z * x;
				if (w <= n) {
					counts[w - 1] += perm(x, y, z);
				}
			}
		}
	}
